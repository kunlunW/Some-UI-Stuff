...
class App extends React.Component {
  ...
  // this is the function that designs the rating mechanism 
  ratingControl(courses, course) {
    let recommendList = this.state.recommend
    if(courses!=='Worst Course Ever! :(' && courses !=='I dont like it! :O' && courses !=='Its an okay class. :|' && courses !== 'Please rate this course!' && !recommendList.includes(course)) {
      recommendList.push(course)
    } 
    else {
      recommendList = recommendList.filter(rl => {
        return !(course === rl)
      })
    }
    this.setState({
      ...
      <>
        ...
        <Tabs defaultActiveKey="search" style={{position: 'fixed', zIndex: 1, width: '100%', backgroundColor: 'white'}}>
          ...
          <Tab eventKey="completedCourses" title="Completed Courses" style={{paddingTop: '5vh'}}>
            {
            this.state.classStatus ?<>processing</> : (<div style={{marginLeft: '9vw'}}>
              
              <PreCourse 
              courses={this.state.allCourses} 
              ratingControl={(data, course) => {this.ratingControl(data, course)}} />
            </div>)
            }
          </Tab>
          ...
          <Tab eventKey="recommendedCourses" title="Recommended Courses" style={{paddingTop: '5vh'}}>
            <div style={{marginLeft: '9vw'}}>
              <RecCourse courses={this.state.recommend}  />
            </div>
          </Tab>


          <Tab eventKey="Scheduler" title="Scheduler (future)" style={{paddingTop: '5vh'}}>
          </Tab>
